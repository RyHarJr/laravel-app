...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <h2 class="text-2xl font-bold text-gray-800">Tambah Periode</h2>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('periode.store') }}" method="POST" class="p-6 space-y-6">
            @csrf

            <div>
                <label for="tahun_akademik" class="block text-sm font-medium text-gray-700 mb-1">Tahun Akademik</label>
                <input type="text" name="tahun_akademik" id="tahun_akademik" value="{{ old('tahun_akademik') }}" class="p-2 w-full rounded-lg border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm @error('tahun_akademik') border-red-500 @enderror" placeholder="Contoh: 2025/2026" maxlength="9" required>
            ...
            <div>
                ...
                <select name="kode_smt" id="kode_smt" class="p-2 w-full rounded-lg border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm @error('kode_smt') border-red-500 @enderror" required>
                    <option value="" disabled selected>Pilih Semester</option>
                    <option value="1" {{ old('kode_smt') == '1' ? 'selected' : '' }}>Ganjil (1)</option>
                    <option value="2" {{ old('kode_smt') == '2' ? 'selected' : '' }}>Genap (2)</option>
                </select>
                @error('kode_smt')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
